...
class LoginActivity : AppCompatActivity() {
    ...
    //アカウント作成時にフラグを立てて、ログイン処理後に名前をFirebaseに保存
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //ログインが成功したか判定する
            if (task.isSuccessful){
                ...
                //？？：FirebaseのRealtime Database内の特定の位置（ここでは、ログインユーザーのIDに対応する位置）を参照するDatabaseReferenceを取得
                val useRef = databaseReference.child(UsersPATH).child(user!!.uid)

                //アカウント作成時かどうかを判定
                if (isCreateAccount){
                    //アカウント作成時は非表示をFirebaseに保存する
                    //ユーザーが入力した名前の取得
